...
Avatar::MoveState WayFinder::findMovDirection2(Strategy strategy) {
    ...
    while (!neighbors.empty()){
        QPoint current = neighbors.front();
        neighbors.pop();
        currentCost = grid[current.y()][current.x()].wayPointNbr + 1;
        for (int i = 1; i < 5; ++i) {
            int x = current.x() + dx[i];
            int y = current.y() + dy[i];
            if((x < 0) || (x >= X_GRID_SIZE) || (y < 0) || (y >= Y_GRID_SIZE)) continue;
            if(grid[y][x].wall) continue;
            if(grid[y][x].wayPointNbr < currentCost) continue;
            grid[y][x].wayPointNbr = currentCost;
            neighbors.push(QPoint(x, y));
